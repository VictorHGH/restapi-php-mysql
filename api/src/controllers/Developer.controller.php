		...
		try {
			...
			if ((gettype($developers) === 'array') && is_array($developers)) {
				if (sizeof($developers) > 0) {
					...
					ResponseMethods::printJSON("NOTFOUD");
				}
			} else {
				ResponseMethods::printError(500, "An undexpected error ocurred...");
			}
		} catch (Exception $ex) {
			// Add exception message to Log.
			ResponseMethods::printError(500, "An unexpected error ocurred...");
		}
	}
}
